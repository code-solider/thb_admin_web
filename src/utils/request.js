...
const request = extend({
  prefix: 'http://122.114.179.152:3001', // prefix
  errorHandler, // 默认错误处理
...
export function getAuth(url,username,password){
  ...
  var pass =  AES.encrypt(url+":"+new Date().getTime(), HmacMD5(password || store.get("user").password,password || store.get("user").password).toString());
